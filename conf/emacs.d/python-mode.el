...
(if py-mode-map
  ...
  (define-key py-mode-map "\C-c\C-c"  'extended-py-execute-buffer)
  (define-key py-mode-map "\C-c\C-m"  'extended-py-execute-buffer-with-args)
  (define-key py-mode-map "\C-c\C-n"  'py-execute-import-or-reload)
...
(and (py-safe (require 'easymenu) t)
	...
	["Execute buffer (Kuso)"       extended-py-execute-buffer t]
	["Execute buffer with args (Kuso)"       extended-py-execute-buffer-with-args t]
...
(defun extended-py-execute-buffer ()
  "run current buffer that contains python code using extended mode."
  (interactive)
  (let (current-buffer-address pybuffer fullcommand commandp cwd current-buffer-dir)
    (setq current-buffer-address (buffer-file-name))
    (setq current-buffer-dir (file-name-directory current-buffer-address))
    (setq cwd default-directory)
    (cd current-buffer-dir)
    (if (stringp current-buffer-address)
	(progn
	  (setq pybuffer (get-buffer-create "*Python*"))
	  (switch-to-buffer pybuffer)
	  (ansi-color-for-comint-mode-on)
	  (add-hook 'after-change-functions 'buffer-change-colorizing t t)
	  (setq fullcommand (concat current-buffer-address))
...
(defun extended-py-execute-buffer-with-args (args)
  "run current buffer that contains python code using extended mode."
  (interactive "sEnter commandlines arguments: ")
  (let (current-buffer-address pybuffer fullcommand commandp current-buffer-dir cwd)
    (setq current-buffer-address (buffer-file-name))
    (setq current-buffer-dir (file-name-directory current-buffer-address))
    (setq cwd default-directory)
    (cd current-buffer-dir)
	...
	(progn
	  ...
	  (add-hook 'after-change-functions 'buffer-change-colorizing t t)
	  (setq fullcommand (concat current-buffer-address))
	  (setq commandp (apply 'make-comint-in-buffer "python" pybuffer *python* nil (list fullcommand args)))
	  )
      (message "This buffer did not visit any file.")
      )
    (cd cwd)
